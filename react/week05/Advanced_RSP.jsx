...
class Advanced_RSP extends Component {
    state = {
        result: '',
        imgCoord: '0',
        score: 0,
        rsqState: 1, // 묵찌빠 : 0, 가위바위보 : 1
        currWinner: '',
    };
    ...
    onClickBtn = (choice) => () => {
        const { imgCoord, rsqState, currWinner } = this.state;
        if (rsqState == 0) { // 묵찌빠
            const myScore = scores[choice];
            let arr = ['바위', '가위', '보'];
            let arrCoords = ['0', '-142px', '-284px'];
            let temp = Math.floor(Math.random()*3);
            const cpuScore = scores[arr[temp]];
            ...
            if (diff == 0) {
                if (currWinner == 'cpu') {
                    this.setState((prevState) => {
                        return {
                            result: '묵찌빠에서 졌습니다!',
                            imgCoord: arrCoords[temp],
                            score: prevState.score - 1,
                            rsqState: 1,
                            currWinner: '',
                        };
                    });
                } else if (currWinner == 'user') {
                    this.setState((prevState) => {
                        return {
                            result: '묵찌빠에서 이겼습니다!',
                            imgCoord: arrCoords[temp],
